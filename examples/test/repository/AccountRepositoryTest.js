...
module.exports.run = async function(repository) {
    logger.logInfo('    - testing AccountRepository...');
    ...
    if (util.isUndefined(repository)) {
        testResults.push(require('../../testStatus.js')(util.ERROR, 'no repository found for Account', 'test.run'));
    } else {
        let metaData = repository.getMetaData();
        if (util.isUndefined(metaData)) {
            testResults.push(require('../../testStatus.js')(util.ERROR, 'no meta data definition found for Account', 'test.run'));
        }

        if (util.isUndefined(repository.getPoolAlias())) {
            testResults.push(require('../../testStatus.js')(util.ERROR, 'no pool alias found in AccountRepository', 'test.run'));
        }
...
async function testFindOne(repository,  testResults) {
    await testUtil.findExampleData(repository.getPoolAlias(), repository.getMetaData()).then(async function(result) {
        if (util.isDefined(result.error)) {
            ...
            testResults.push(require('../../testStatus.js')(util.WARN,  'AccountRepository: no test data found in table CA_ACCOUNT_T', util.FIND_ONE));
            return false;
...
async function testFind(repository, testResults) {
    await testUtil.findExampleData(repository.getPoolAlias(), repository.getMetaData(), 10).then(async function(result) {
        if (util.isDefined(result.error)) {
            ...
            testResults.push(require('../../testStatus.js')(util.WARN,  'AccountRepository: no test data found in table CA_ACCOUNT_T', util.FIND));
        } else if (util.isDefined(result.result)) {
            let whereList = testUtil.getFindTestWhereLists(repository, result.result);
            if (whereList.length === 0) {
                testResults.push(require('../../testStatus.js')(util.WARN,  'FiscalYearFunctionControlRepository: no test data found in table FP_FSCL_YR_CTRL_T', util.FIND));
            } else {
...
async function testGetAll(repository, testResults) {
    ...
    if (await testUtil.isGetAllTestAllowed(repository, testResults)) {
        ...
        testResults.push(require('../../testStatus.js')(util.WARN,  'not performing getAll() test on Account(CA_ACCOUNT_T)', util.GET_ALL));
    }
};
...
async function testExists(repository,testResults) {
    return await testUtil.findExampleData(repository.getPoolAlias(), repository.getMetaData()).then(async function(result) {
        if (util.isDefined(result.error)) {
            ...
            testResults.push(require('../../testStatus.js')(util.WARN,  'AccountRepository: no test data found in table CA_ACCOUNT_T', util.EXISTS));
        } else if (util.isDefined(result.result)) {
            let pkfields = repository.getMetaData().getPrimaryKeyFields();
